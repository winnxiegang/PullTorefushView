...
public class PullToRefreshView extends ViewGroup {
    ...
    public PullToRefreshView(Context context, AttributeSet attrs) {
        ...
        final int type = a.getInteger(R.styleable.PullToRefreshView_lrefresh, TYPE_MY);
        a.recycle();*/
...
//                setTargetOffsetTop(0, true);
    ...
    public void setRefreshing(boolean refreshing) {
        if (mRefreshing != refreshing) {
            //// TODO: 2017/5/8
            setRefreshing(refreshing, false, true);
        }
    }

    private void setRefreshing(boolean refreshing, final boolean notify, boolean isRefreshSuccess) {
        if (mRefreshing != refreshing) {
            mNotify = notify;
            ensureTarget();
            ...
            if (mRefreshing) {
                ...
                if (isRefreshSuccess) {
                    postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            mBaseRefreshView.refreshSuccess();
                            animateOffsetToStartPosition();
                        }
                    }, 500);
                } else {
...
//            mBaseRefreshView.stop();
    ...
    private void setTargetOffsetTop(int offset, boolean requiresUpdate) {
        Log.e("offset=" + offset, "requiresUpdate" + requiresUpdate);
        mTarget.offsetTopAndBottom(offset);
        mBaseRefreshView.offsetTopAndBottom(offset, requiresUpdate);
        mCurrentOffsetTop = mTarget.getTop();
    ...
    public void onDestroy() {
        mRefreshView.setImageDrawable(null);
    }
